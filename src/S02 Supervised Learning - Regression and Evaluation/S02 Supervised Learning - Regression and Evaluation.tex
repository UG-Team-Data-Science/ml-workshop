...
\date[Feb 6\textsuperscript{th} 2026]{Friday, February 6\textsuperscript{th} 2026}
...
\label{sec:org9f6e625}
\begin{frame}[label={sec:org4f99e08}]{Introduction to Regression}
\begin{definition}[Regression]\label{sec:orgb36f0d3}
\pause
...
\label{sec:orgdc904f4}
\begin{frame}[label={sec:org5f48255}]{Simple Linear Regression}
\begin{definition}[Simple Linear Regression]\label{sec:orga9f5655}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org8b5c5cd}]{Simple Linear Regression}
...
\end{frame}
\begin{frame}[label={sec:orgd689b2e}]{Assumptions of Simple Linear Regression}
\begin{itemize}[<+->]
...
\label{sec:orga019f9d}
\begin{frame}[label={sec:org9bce8b5}]{Evaluation Metrics for Regression}
\begin{definition}[Common Metrics for Regression]\label{sec:orgf49a57b}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org1fa9477}]{Mean Absolute Error (MAE)}
\begin{definition}[Mean Absolute Error (MAE)]\label{sec:org9a3c227}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:orgbb33f2d}]{Mean Squared Error (MSE)}
\begin{definition}[Mean Squared Error (MSE)]\label{sec:org3c26192}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org8015413}]{Root Mean Squared Error (RMSE)}
\begin{definition}[Root Mean Squared Error (RMSE)]\label{sec:org6e93430}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org7d438c3}]{R-squared (\(R^2\))}
\begin{definition}[R-squared (\(R^2\))]\label{sec:org19f9f24}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:orga9622cf}]{Adjusted R-squared}
\begin{definition}[Adjusted R-squared]\label{sec:orgcf76cc3}
\pause
...
\label{sec:org3efe199}
\begin{frame}[label={sec:org4b6602b}]{Regression Diagnostics}
\begin{definition}[Regression Diagnostics]\label{sec:orge30d3ea}
\pause
\alert{Regression Diagnostics} is the process of verifying that a regression model's assumptions are met. A high \(R^2\) does not guarantee a valid model; if assumptions are violated, predictions and confidence intervals can be unreliable.
\pause
The primary tool for diagnostics is analyzing the \alert{Residuals} (the errors: \(y - \hat{y}\)).
\end{definition}
\end{frame}
\begin{frame}[label={sec:org454b566}]{Key Diagnostic Checks}
\begin{definition}[Key Diagnostic Checks]\label{sec:org0441068}
\pause
\begin{itemize}[<+->]
\item \alert{Linearity}: Residuals should be randomly scattered around 0. A U-shape indicates non-linearity.
\item \alert{Homoscedasticity}: Residuals should have constant variance. A "funnel" shape indicates heteroscedasticity.
\item \alert{Normality}: Residuals should follow a Normal distribution (check with Q-Q plot).
\item \alert{Independence}: Residuals should not be correlated (important for time-series data).
\end{itemize}
\end{definition}
\end{frame}
\begin{frame}[label={sec:org5589815}]{Residual Plot}
\begin{definition}[Residual Plot]\label{sec:org396cb13}
\pause
A \alert{Residual Plot} graphs the residuals (\(y - \hat{y}\)) against the predicted values (\(\hat{y}\)).
\pause
Interpretation:
\begin{itemize}
\item \alert{Random scatter around 0}: Assumptions are likely met.
\item \alert{Funnel shape} (variance increases): Heteroscedasticity—consider log-transforming the target.
\item \alert{U-shape or pattern}: Non-linearity—consider polynomial features or non-linear models.
\end{itemize}
\end{definition}
\end{frame}
\begin{frame}[label={sec:orgd9a6854}]{Q-Q Plot (Normality Check)}
\begin{definition}[Q-Q Plot]\label{sec:org50b8b94}
\pause
A \alert{Q-Q Plot} (Quantile-Quantile Plot) compares the distribution of residuals against a theoretical Normal distribution.
\pause
Interpretation:
\begin{itemize}
\item \alert{Points on the diagonal line}: Residuals are normally distributed.
\item \alert{Tails curving away}: Heavy tails or skewness—consider transformations or outlier removal.
\end{itemize}
\end{definition}
\end{frame}
\begin{frame}[label={sec:org8905b7e}]{Influential Points (Cook's Distance)}
\begin{definition}[Cook's Distance]\label{sec:org39a3e95}
\pause
\alert{Cook's Distance} measures how much the regression coefficients change when a single observation is removed. It identifies influential outliers that disproportionately affect the model.
\pause
A high Cook's Distance (typically \(> 4/n\)) indicates that the observation has a strong influence on the fitted model and should be investigated.
\end{definition}
\end{frame}
\section{Multiple Linear Regression}
\label{sec:org4b98bfc}
\begin{frame}[label={sec:org11e832e}]{Multiple Linear Regression}
\begin{definition}[Multiple Linear Regression]\label{sec:org9b2d071}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org1d8ba0a}]{Assumptions of Multiple Linear Regression}
\begin{definition}[Assumptions of Multiple Linear Regression]\label{sec:org0ddecc1}
\pause
...
\label{sec:orgcd5fbaa}
\begin{frame}[label={sec:org1836379}]{How do Machine Learning Models Learn?}
\begin{definition}[How do Machine Learning Models Learn?]\label{sec:org0304274}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org9353970}]{Cost Function}
\begin{definition}[Cost Function]\label{sec:org1a99b85}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org888997b}]{Cost Function: Mean Squared Error (MSE)}
\begin{definition}[Cost Function: Mean Squared Error (MSE)]\label{sec:org0d69b4b}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org4fbf3aa}]{Cost Function: Mean Absolute Error (MAE)}
\begin{definition}[Cost Function: Mean Absolute Error (MAE)]\label{sec:orgc859ba9}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:orgafab291}]{Gradient Descent}
\begin{definition}[Gradient Descent]\label{sec:orgc3c0d73}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:orgeb557e2}]{Gradient Descent Algorithm}
\begin{definition}[Gradient Descent Algorithm]\label{sec:org7aca36e}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:orgf558ebc}]{Overfitting and Underfitting}
\begin{definition}[Overfitting and Underfitting]\label{sec:org2570ef4}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org930ac5e}]{Regularization}
\begin{definition}[Regularization]\label{sec:org6ced9db}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org6a3cc8b}]{Types of Regularized Regression}
\begin{definition}[Types of Regularized Regression]\label{sec:orgce8d117}
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org927410d}]{Lasso Regression (L1 Regularization)}
\begin{definition}[Lasso Regression (L1 Regularization)]\label{sec:orga0ac71d}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:orgfef541e}]{Ridge Regression (L2 Regularization)}
\begin{definition}[Ridge Regression (L2 Regularization)]\label{sec:org1a65ad7}
\pause
...
\label{sec:org1845df1}
\begin{frame}[label={sec:org9598cec}]{Nonlinearities in Regression}
\begin{definition}[Nonlinearities in Regression]\label{sec:orge311b6a}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:orgc9e69be}]{Types of Nonlinear Regression Models}
\begin{definition}[Types of Nonlinear Regression Models]\label{sec:org7475ebc}
...
\label{sec:org1957326}
\begin{frame}[label={sec:org9cc0ecb}]{Overfitting, Underfitting, and Hyperparameter Tuning}
\begin{definition}[Overfitting]\label{sec:org7401f70}
\pause
\alert{Overfitting} occurs when a model learns the noise in the training data instead of the underlying pattern, resulting in poor generalization to unseen data. It typically happens when the model is too complex relative to the amount of training data.
\end{definition}
\end{frame}
\begin{frame}[label={sec:orga2eb935}]{Underfitting}
\begin{definition}[Underfitting]\label{sec:orgbbc4b75}
\pause
\alert{Underfitting} occurs when a model is too simple to capture the underlying pattern in the data, resulting in poor performance on both training and unseen data. It typically happens when the model is not complex enough relative to the data.
\end{definition}
\end{frame}
\begin{frame}[label={sec:org7bdb4de}]{Bias-Variance Tradeoff}
\begin{definition}[Bias-Variance Tradeoff]\label{sec:orge9e12af}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org9cd0a57}]{Learning Curves}
\begin{definition}[Learning Curves]\label{sec:org8435932}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org2077f17}]{Hyperparameter Tuning}
\begin{definition}[Hyperparameter Tuning]\label{sec:org8789ef2}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:org465d02e}]{Techniques for Hyperparameter Tuning}
\begin{definition}[Techniques for Hyperparameter Tuning]\label{sec:orgeed016d}
