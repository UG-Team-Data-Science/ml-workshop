...
\label{sec:orgfa4b015}
\begin{frame}[label={sec:orgbdcaabd}]{What is an Artificial Neural Network?}
\begin{definition}[Definition]\label{sec:org1ab2f4a}
\pause
...
\pause
\end{definition}
\begin{example}[Key Properties]\label{sec:orgb971635}
\begin{itemize}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgd95f9b3}]{Key Components of a Neural Network}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org089ab53}]{Biological Inspiration}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgca28863}]{The Perceptron (1958)}
\begin{definition}[The Simplest Neural Network]\label{sec:org310cae6}
\pause
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc0c1cda}]{The XOR Problem}
...
\pause
\end{block}
\begin{example}[The Limitation]\label{sec:org59d29f4}
\begin{itemize}[<+->]
...
\label{sec:org5d3f846}
\begin{frame}[label={sec:org880307e}]{From One to Many}
\begin{definition}[Stacking Neurons]\label{sec:orgec20826}
\pause
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org79ba5a5}]{Activation Functions}
...
\end{block}
\begin{example}[Common Activation Functions]\label{sec:orgd817691}
\pause
\begin{center}
\begin{tabular}{llll}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org41cf858},fragile]{Forward Propagation}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org212c40f}]{Network Capacity: Width vs Depth}
...
\pause
\end{block}
\begin{example}[Trade-offs]\label{sec:org1499594}
\begin{itemize}[<+->]
...
\label{sec:orge394614}
\begin{frame}[label={sec:orgcee9169}]{The Goal of Training}
...
\pause
\end{block}
\begin{example}[Three Components]\label{sec:org83d1a93}
\begin{enumerate}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org0c0fba7}]{Loss Functions}
\begin{definition}[The Scorecard]\label{sec:orgefaf011}
\pause
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org3478b90}]{Stochastic Gradient Descent (SGD)}
...
\end{block}
\begin{definition}[Stochastic Gradient Descent]\label{sec:orgd23dda0}
\pause
...
\end{definition}
\end{frame}
\begin{frame}[label={sec:orge5ded9d}]{Backpropagation}
...
\pause
\end{block}
\begin{example}[The Process]\label{sec:orgee60b7b}
\begin{enumerate}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgced0470}]{The Vanishing Gradient Problem}
...
\pause
\end{block}
\begin{example}[The Consequence]\label{sec:orgfe3c32f}
\begin{itemize}[<+->]
...
\label{sec:orge829fdd}
\begin{frame}[label={sec:org8ba08a6}]{Weight Initialization}
...
\pause
\end{block}
\begin{example}[Strategies]\label{sec:org4b01a4a}
\begin{itemize}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org87cc935}]{Advanced Optimizers}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org5d89534}]{Regularization: Dropout}
\begin{definition}[Fighting Overfitting]\label{sec:org4d63258}
\pause
\alert{Dropout} randomly "kills" (sets to zero) a percentage of neurons during training.
\pause
\end{definition}
\begin{example}[Why It Works]\label{sec:org2af5a66}
\begin{itemize}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org82a17ab}]{Regularization: Batch Normalization}
\begin{definition}[Stabilizing Training]\label{sec:orgb5b57f6}
\pause
\alert{Batch Normalization} normalizes layer activations per mini-batch during training (with learnable scale and shift).
\pause
\end{definition}
\begin{example}[Benefits]\label{sec:orgcb9133c}
\begin{itemize}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org30f3510},fragile]{Early Stopping}
...
\pause
\end{block}
\begin{example}[How It Works]\label{sec:orgba5a44a}
\begin{itemize}[<+->]
...
\label{sec:orge369199}
\begin{frame}[label={sec:org25ab4e9}]{Why PyTorch?}
...
\end{enumerate}
\end{block}
\begin{example}[PyTorch Advantages]\label{sec:orgcc557c1}
\pause
\begin{itemize}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgabf51eb},fragile]{Tensors: The Foundation}
 \begin{definition}[NumPy Arrays on Steroids]\label{sec:org4ad692c}
\pause
...
\pause
\end{definition}
\begin{example}[Key Operations]\label{sec:org837f6ac}
\begin{itemize}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgfb3215d},fragile]{Defining a Network (\texttt{nn.Module})}
 \begin{block}{The Blueprint}
\pause
In PyTorch, a network is a Python class inheriting from \texttt{nn.Module}:
\pause
\end{block}
\begin{example}[Two Required Methods]\label{sec:orgbd04975}
\begin{enumerate}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org2fd7327},fragile]{The Training Loop}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org4a3b823},fragile]{PyTorch Outputs and Loss Pairings}
...
\end{itemize}
\end{block}
\begin{example}[Inference Rule]\label{sec:orgc81bec3}
\pause
\begin{itemize}[<+->]
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org3a48938},fragile]{Loss Functions and Optimizers}
 \begin{example}[Loss Functions]\label{sec:orgc448c4a}
\begin{center}
\begin{tabular}{ll}
Task & Loss Class\\
\hline
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8bdca76},fragile]{Evaluation Mode}
...
\end{frame}
\section{Summary}
\label{sec:org90ac091}
\begin{frame}[label={sec:orgdf0017d}]{Key Takeaways}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgecb1a8c}]{Next Section Preview}
